...
- Estrutura:
  ...
  - `src/envstencil/cli.py` — comandos `envstencil generate [SOURCE] [-o OUTPUT] [-p PLACEHOLDER] [-f/--force] [-a/--append] [-b/--collapse-blank-lines]` e `envstencil check [FILE1] [FILE2] [-e/--example FILE] [--diff/--dif]`
...
- `check` aceita até 2 posicionais: 0 args → `.env`/`.env.example`; 1 arg → `FILE1`/`FILE1 + ".example"`; 2 args → exatamente `FILE1`/`FILE2` (sem convenção). `-e/--example` continua como forma compatível de dar o 2º arquivo; `FILE2` + `--example` juntos → `click.UsageError` (exit 2). A resolução mora no CLI, não no core

...
- Stack: Python puro (stdlib `re`, `pathlib`, `dataclasses`, `tomllib`, `os`) + `click` para CLI
...
- Estrutura:
  - `src/envstencil/core.py` — parsing do `.env` (`parse_env_file`), geração do stencil (`render_stencil`, `generate_example`), atualização incremental (`append_missing_variables` → `AppendResult`) e comparação de chaves (`compare_env_files` → `EnvComparison`); helpers `get_keys` / `_ordered_keys`. Genérico: NÃO importa `click`, `tomllib`, `config.py`, XDG nem `pyproject`
  - `src/envstencil/config.py` — modelo de config (`EnvStencilConfig` + seções `GlobalConfig`/`GenerateConfig`/`CheckConfig`, todo campo `... | None`), parsing TOML (`parse_config`, `load_toml`), merge campo-a-campo (`merge_config`; não-`None` vence, inclusive `False`), descoberta de fontes (`load_config(cwd, explicit_config)` empilha defaults → config do usuário XDG → `[tool.envstencil]` do `pyproject.toml` → `.envstencil.toml` → arquivo de `--config`; `pyproject.toml` e `.envstencil.toml` são procurados via `_find_upwards` do `cwd` até a raiz, de forma independente, só a ocorrência mais próxima de cada nome — sem empilhar ancestrais, sem parada em `.git`; `--config` e XDG não participam da busca) e resolução por comando (`resolve_check_config`/`resolve_generate_config` → `Resolved*Config` sem `None`, via `_resolve` com checagem explícita de `None`). `ConfigError(ValueError)` para conteúdo/tipo inválido; erro de filesystem propaga separado
  - `src/envstencil/cli.py` — opção global `--config CAMINHO` (carrega a config uma vez em `ctx.obj["config"]`); comandos `envstencil generate [SOURCE] [-o OUTPUT] [-p PLACEHOLDER] [-f/--force/--no-force] [-a/--append] [-b/--collapse-blank-lines]` e `envstencil check [FILE1] [FILE2] [-e/--example FILE] [--diff/--no-diff]`
...
- `compare_env_files` (core, comando `check`): **read-only** — nunca cria/altera arquivo nem escreve `.tmp`. Compara só nomes de chave (nunca valores), reusa `parse_env_file` nos dois arquivos, `unknown`/aspa não fechada em qualquer um → aborta. `EnvComparison` com `missing_in_source`/`missing_in_example` (ordem do arquivo de origem de cada chave, via `_ordered_keys`) e `is_synced`. `# envstencil:keep` não influencia. Não corrigir divergências automaticamente (nada de chamar `--append` sozinho). API pública de `compare_env_files(source, example)` inalterada — é genérica para dois dotenv quaisquer
- `check` aceita até 2 posicionais: 0 args → arquivos resolvidos da config (`.env`/`.env.example` por padrão); 1 arg → `FILE1`/`FILE1 + ".example"` (o `file2` da config NÃO entra); 2 args → exatamente `FILE1`/`FILE2` (config ignorada). `-e/--example` continua como forma compatível de dar o 2º arquivo; precedência do 2º arquivo: `FILE2` > `--example` > config. `FILE2` + `--example` juntos → `click.UsageError` (exit 2). A resolução mora no CLI, não no core
- Sistema de config integrado no `check`/`generate`: cada subcomando lê `ctx.obj["config"]` (NÃO chama `load_config` de novo), roda `resolve_check_config`/`resolve_generate_config` e então aplica os args/flags explícitos por cima — CLI sempre vence. Cadeia de precedência completa: `CLI > --config > .envstencil.toml > [tool.envstencil] do pyproject > config global do usuário > defaults`. O CLI é a única camada fora do `merge_config`. Helpers de resolução da camada CLI (`_resolve_check_inputs`, `_resolve_generate_inputs`) ficam no `cli.py`, não no core. NUNCA mutar o `EnvStencilConfig` de `ctx.obj` — produzir valores/objetos locais. `ConfigError` de config inválida → `_InputError` (exit 2) ANTES de rodar o core; erro de dotenv mantém os exit codes de sempre. Sem nenhum arquivo de config, comportamento idêntico ao anterior
- `--force/--no-force` (generate) e `--diff/--no-diff`/`--dif/--no-dif` (check) são flag pairs tri-state do Click, `default=None`: `None` → usa `config.generate.force` / `config.check.diff`; `True`/`False` explícito vence a config. Nunca usar `or` — checagem explícita de `None`. `--append` + `--force` **explícito** (`force is True`) juntos → `click.UsageError`; `force` vindo só da config não dispara isso
- `generate`: no core, `generate_example`/`append_missing_variables` seguem recebendo `source`/`destination` explícitos; o CLI converte `ResolvedGenerateConfig.file1/file2` → `source`/`output`. Origem/destino explícitos vencem a config; só a origem explícita → deriva `<source>.example` (não puxa o `file2` da config). `SOURCE` perdeu `exists=True`/`default` no Click (o default vem da config); origem inexistente vira `ClickException` no `generate_example`
- `pyproject.toml` / `.envstencil.toml` procurados com `_find_upwards(filename, cwd=None)` (helper genérico e testável isoladamente em `config.py`): sobe `cwd` → pais até `current.parent == current`, retorna o 1º candidato cujo `.exists()` é `True` (NÃO `is_file()` — diretório com o nome conta como "encontrado" e deixa a leitura levantar `IsADirectoryError`), senão `None`. Sem `resolve()`, sem tratamento de symlink. `load_pyproject_config`/`load_project_config` usam o helper e retornam `EnvStencilConfig()` quando é `None`; toda a semântica de parsing/erro anterior é preservada. `--config` e XDG continuam fora da busca
- Fora de escopo do sistema de config: resolução de caminhos relativos do TOML pela localização do arquivo (hoje ainda relativos ao dir de execução), empilhar vários `.envstencil.toml`/`pyproject.toml` ancestrais, parada em `.git`, suporte a monorepo/workspace, profiles, includes, env vars como camada, aliases novos além dos existentes, formatos não-TOML, mudança de semântica no core
